...
class SpikingVGG11(nn.Module, ISNNModel):
    ...
    def __init__(self, in_channels: int, out_classes: int, dropout: float = 0.5):
        ...
        self.network = nn.Sequential(
            layer.Conv2d(in_channels, 64, kernel_size=3, padding=1, bias=False),
            layer.BatchNorm2d(64),
            neuron.LIFNode(tau=2.0, v_threshold=0.5, surrogate_function=sg),
            layer.MaxPool2d(2, 2),
            ...
            layer.Flatten(),
            layer.Dropout(dropout),
            layer.Linear(512, 4096, bias=False),
