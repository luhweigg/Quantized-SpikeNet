...
class CompactSpikingCNN(nn.Module):
    ...
    """
    def __init__(self, in_channels: int = 1, num_classes: int = 10, neuron_type: str = "LIF", **neuron_kwargs):
        super().__init__()
        self.features = nn.Sequential(
            SpikingConvBlock(in_channels, 32, kernel_size=3, padding=1, neuron_type=neuron_type, **neuron_kwargs),
            layer.MaxPool2d(2, 2),
        ...
        self.classifier = nn.Sequential(
            layer.AdaptiveAvgPool2d((8, 8)),
            layer.Flatten(),
            SpikingLinearBlock(64 * 8 * 8, 512, neuron_type=neuron_type, **neuron_kwargs),
