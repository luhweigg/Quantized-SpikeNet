...
from src.models import CifarCompactConvSNN, DeepConvSNN, GenericMLPSNN


def test_deepconv_snn_accepts_downsampled_cifar10_shape():
    model = DeepConvSNN(in_channels=2, out_classes=10)
...
def test_quantization_returns_metadata():
    model = GenericMLPSNN(input_size=2312, hidden_size=256, output_size=10)
...
def test_cifar_compact_and_deep_shapes_match_for_cifar10():
    compact = CifarCompactConvSNN(in_channels=2, out_classes=10)
    deep = DeepConvSNN(in_channels=2, out_classes=10)
    x = torch.zeros(4, 2, 2, 32, 32)
