...
def test_snn_state_reset():
    ...
    assert not torch.equal(out_forward_a, out_forward_b), "Echec: Le reseau n'accumule pas l'etat temporel."

    functional.reset_net(model)
    out_forward_c = model(x)

    assert torch.equal(out_forward_a, out_forward_c), "Echec: functional.reset_net() ne nettoie pas le reseau."
